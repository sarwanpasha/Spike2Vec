...
for lp_tmp in arr_all_datasets:
    ...
    read_path_1 = "/alina-data1/sarwan/IEEE_BigData/Dataset/kmers_frequency_vectors_total_" + str(lp_tmp) + ".csv"    
...
sss = ShuffleSplit(n_splits=1, test_size=0.95)
...
print("Ridge regression Starts here!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
...
Ridge_Regression_data = sel_.transform(pd.DataFrame(X_test).fillna(0))
...
for clust_ind in range(len(number_of_clusters)):
    ...
    write_path_11 = "/alina-data1/sarwan/IEEE_BigData/Dataset/ridge_freq_vec_hard_kmeans_clustering_k_" + str(clust_num) + ".csv"
    ...
    write_path_11 = "/alina-data1/sarwan/IEEE_BigData/Dataset/attributes_ridge_freq_vec_hard_kmeans_clustering_k_" + str(clust_num) + ".csv"
    with open(write_path_11, 'w', newline='') as file:
        writer = csv.writer(file)
